...
//	@Query("select r from Requerimiento r fetch r.aplicacion a  where r.requerimiento like %?1%  ")
	...
	@Query(value = "select * from Requerimiento where aplicacion like %:aplicacion% and (:requerimiento is null or requerimiento like %:requerimiento%) and (:estado is null or estado like %:estado%) ", nativeQuery = true)
